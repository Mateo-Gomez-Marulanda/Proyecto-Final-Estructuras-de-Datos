...
public class DashboardController {
    ...
    @FXML
    private Label numInmuebles;
    @FXML
    private Label numClientes;
    @FXML
    private Label numVisitas;
    @FXML
    private Label numAlertas;

    @FXML
    private TableView<PropertyChange> tablaUltimosMovimientos;
    @FXML
    private TableColumn<PropertyChange, String> colAccionFecha;
    @FXML
    private TableColumn<PropertyChange, String> colAccionTipo;
    @FXML
    private TableColumn<PropertyChange, String> colAccionDetalle;
    @FXML
    private TableColumn<PropertyChange, String> colAccionResponsable;
    ...
    private void configurarTablaAcciones() {
        colAccionFecha.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getDateTime().toString()));
        colAccionTipo.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getChangeType().toString()));
        colAccionDetalle.setCellValueFactory(data -> new SimpleStringProperty(
        ...
        colAccionResponsable
                .setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getResponsiblePerson()));
    }
    ...
    @FXML
    public void irARegistrarInmueble() {
        navegarA("/proyectofinal/views/inmuebles-content.fxml");
    }

    @FXML
    public void irARegistrarCliente() {
        navegarA("/proyectofinal/views/clientes-content.fxml");
    }

    @FXML
    public void irAAgendarVisita() {
        navegarA("/proyectofinal/views/visitas-content.fxml");
    }

    @FXML
    public void irARegistrarOperacion() {
        navegarA("/proyectofinal/views/operaciones-content.fxml");
    }

    @FXML
    public void irAAlertas() {
        navegarA("/proyectofinal/views/alertas-content.fxml");
    }
    ...
    private void navegarA(String path) {
        try {
            ...
            javafx.scene.layout.StackPane parent = (javafx.scene.layout.StackPane) numInmuebles.getScene()
                    .lookup("#contenidoCentral");
            if (parent != null)
                parent.getChildren().setAll(vista);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void abrirVentanaReportes() {
        try {
            // Se usa el prefijo /proyectofinal/views/ manteniendo consistencia con tus
            // layouts
            ...
            new Alert(Alert.AlertType.ERROR, "Error al abrir el módulo de analítica: " + e.getMessage(), ButtonType.OK)
                    .showAndWait();
            e.printStackTrace();
        }
    }
